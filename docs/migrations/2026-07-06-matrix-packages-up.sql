...
CREATE TABLE IF NOT EXISTS `matrix_packages` (
  ...
  `delivery_status_code` VARCHAR(50) NULL,
  ...
  KEY `idx_matrix_packages_delivery_status` (`delivery_status_code`),
...
INSERT INTO `config_dict_types` (`type_key`, `type_name`, `description`, `enabled`, `is_builtin`)
SELECT 'matrix_package_health', '矩阵包健康度', '仅对运营中矩阵包生效，第一版手动维护，后续可按数据规则自动判断。', 1, 1
...
WHERE NOT EXISTS (SELECT 1 FROM `config_dict_types` WHERE `type_key` = 'matrix_package_delivery_status');
...
INSERT INTO `config_dict_items` (`type_key`, `item_code`, `item_name`, `sort_order`, `enabled`, `color`, `remark`)
SELECT 'matrix_package_status', 'DELIVERING', '运营中', 40, 1, 'processing', '当前正在消耗预算进行运营'
WHERE NOT EXISTS (
