...
DOWN_SQL="$SCRIPT_DIR/2026-03-28-project-management-v2-permissions-down.sql"
VERIFY_SQL="$SCRIPT_DIR/2026-03-28-project-management-v2-permissions-verify.sql"

if [[ ! -f "$DOWN_SQL" ]]; then
  echo "[ERROR] DOWN sql not found: $DOWN_SQL"
  exit 1
fi
...
echo "[WARN] Rolling back migration: $(basename "$DOWN_SQL")"
run_with_mysql_if_available "$DOWN_SQL" "$(basename "$DOWN_SQL")"

echo "[INFO] Running verification: $(basename "$VERIFY_SQL")"
run_with_mysql_if_available "$VERIFY_SQL" "$(basename "$VERIFY_SQL")"

echo "[DONE] Permission rollback finished."
