...
function createPostPolicy({
  ...
  const policyObj = {
    ...
    conditions: [
      { bucket: bucketName },
      ['eq', '$key', objectKey],
      ['content-length-range', 0, Math.max(1024, Number(maxFileSize) || 50 * 1024 * 1024)],
...
function getOssConfigFromEnv() {
  ...
  const maxFileSize = Math.max(1024, Number(process.env.ALIYUN_OSS_MAX_FILE_SIZE || 50 * 1024 * 1024))
